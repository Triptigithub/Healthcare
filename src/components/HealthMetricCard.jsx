...
import { HealthData, ZoomIn, ArrowRight } from '../data';
    ...
    <div className="flex-grow">
      <div className="flex">
        ...
        <div className="flex p-2 justify-between items-center">
          <h4 className="font-semibold text-cl-text-primary text-sm">{metric.title}</h4>
        </div>
      </div>
      <p className="text-xs text-cl-text-muted mt-2">{metric.date}</p>
      <div className="w-full bg-gray-200 my-2 rounded-full h-1.5">
        <div
          className={`${metric.progressColor} h-1.5 rounded-full`}
      ...
      <div className="grid grid-cols-1 md:grid-cols-9 gap-6">
        ...
        <div className="md:col-span-5 bg-cl-background-light p-6 rounded-xl 2xl:rounded-3xl flex justify-center items-center relative min-h-[380px] md:min-h-[460px]">
          <div className="relative">
            <div className="max-h-[320px] md:max-h-[420px] object-contain">
              <ModelViewer modelPath={modelFile} />
            </div>
            <img className="rotate-180 ml-8 mt-4 w-[80%] h-[8%]" src='../../long-arrow.png' alt="Arrow" />
          </div>
